...
export const katago = {
  candidates: (board:number[][], toMove:'B'|'W', rank:Rank, n:number, strategy:Strategy, history:HistoryMove[]=[], maxVisits=400) =>
    post('/candidates', {board, toMove, rank, n, strategy, history, maxVisits}) as Promise<{moves:Candidate[], meta:any}>,
  ...
  evaluate: (board:number[][], move:{x:number,y:number}, toMove:'B'|'W', rank:Rank='10k', history:HistoryMove[]=[], maxVisits=400) =>
    post('/evaluate', {board, move, toMove, rank, history, maxVisits}) as Promise<{winrate:number, scoreLead:number, ownership:number[][]}>,
