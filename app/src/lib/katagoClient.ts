...
export const katago = {
  candidates: (board:number[][], toMove:'B'|'W', rank:Rank, n:number, strategy:Strategy, maxVisits=400) =>
    post('/candidates', {board, toMove, rank, n, strategy, maxVisits}) as Promise<{moves:Candidate[], meta:any}>,
  genmove: (board:number[][], toMove:'B'|'W', rank:Rank, maxVisits=400) =>
    post('/genmove', {board, toMove, rank, maxVisits}) as Promise<{move:{x:number,y:number,pass:boolean}, winrate:number, scoreLead:number}>,
  evaluate: (board:number[][], move:{x:number,y:number}, toMove:'B'|'W', maxVisits=400) =>
