...
const candidatesBody = z.object({
  ...
  maxVisits: z.number().min(1).max(2000).default(150) // search depth; mock scales noise with it
})
app.post('/candidates', async(req, reply)=>{
  ...
  if(engine.mode==='real'){
    // TODO: wire real query with maxVisits; fallback to mock
  }
...
app.post('/genmove', async(req, reply)=>{
  ...
  const {board, rank, maxVisits} = p.data
